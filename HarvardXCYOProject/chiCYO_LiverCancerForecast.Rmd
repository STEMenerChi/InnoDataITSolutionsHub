...
subtitle: "HarvardX Data Science, Machine Learning"
author: "C.T. Dinh"
...
knitr::read_chunk("chiCYOliverCancerForecast.R")
knitr::opts_chunk$set(fig.path = "C:/Chi/HarvardXCYO/images/")
...
```{r Prep Packages, message=FALSE, warning=FALSE}
...
```{r Plot Train & Validation, echo=TRUE, message=FALSE, warning=FALSE  }
...
```{r Model 2 Fit & Plot, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r All Models fit & coef, echo=TRUE, message=FALSE, warning=FALSE}
...
legend("topright", c("Observed(train)", "ARIMA(0,0,0)", "ARIMA(0,0,1)", "ARIMA(0,0,0) Fourier w/ K=1"), lty = 8, col = c("#00B7C7", "green", "#FFA300", "purple"), cex = 0.8)
```
...
```{r plot model 4  & others, echo=TRUE, message=FALSE, warning=FALSE}
# Plot fitted models
plot(trainL.ts, col = "#00B7C7", main = "Fitted Models")
lines(fitted(autoarima.Model1), col = "green", lwd = 2)
lines(fitted(MA1.model2), col = "#ffa300", lwd = 2)
...
legend("topright", c("Observed (Train)", "ARIMA(0,0,0)", "ARIMA(0,0,1)", "ARIMA(0,0,0) w/ Fourier K=1", "ARIMA(0,0,0) Transformed"), lty = 8, col = c("darkgreen", "green", "#FFA300", "purple", "blue"), cex = 0.8)
```
...
```{r Model 5a fit & forecast, echo=TRUE, message=FALSE, warning=FALSE}
...
ses.fit.model5b.em[, c("RMSE", "MAPE")]
```
...
Plot fitted models
```{r Model 6 - fitted models plot, echo=FALSE, message=FALSE, warning=FALSE}
plot(trainL.ts, col = "#00B7C7", main = "Fitted Models")
lines(fitted(autoarima.Model1), col = "green", lwd = 2)
lines(fitted(MA1.model2), col = "#ffa300", lwd = 2)
lines(fitted(fit.fourier.model3), col = "purple", lwd = 2)
...
legend("topright", c("Observed (valid.ts)", "NNetAR"), lty = 8, col = c("#FF7F50", "black"), cex = 0.9)
```
...
```{r avg Max & Min annually, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r compare data tables, echo=FALSE, message=FALSE, warning=FALSE}

# look at table, pc side by side
kbl(caption = "Actual Cases 2009-2021 ---vs-- Forecast Cases 2022-2026", list(table, pc)) %>%
  kable_classic_2(full_width = F, c("striped", "hover"))
```
...
Examine the Actual Cases 2009-2021 and Forecast Cases 2022-2026 above side by side,  
Based on  the actual **avg_count cases and the percent change ** with the forecast **avg.max cases and percent change**, our model prediction is pretty accurate. 
...
## References
    ...
    Hyndman, Rob J. and Athanasopoulos, George, Forecasting: Principles and Practice (2nd ed), 8.1 Stationarity 
    and differencing, https://otexts.com/fpp2/stationarity.html.
